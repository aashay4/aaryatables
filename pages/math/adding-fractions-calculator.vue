    ...
    <div class="w3-col l8 s12">
      ...
      <div class="w3-margin w3-white">
        ...
        <div class="w3-container w3-padding">
              <a href="/">Home</a> &rsaquo; <a href="/math">Mathematics</a> &rsaquo; Add fractions<hr>
          <h1 class="w3-text-teal"><b>Adding fractions calculator</b></h1>
          <Adsense
...
<div class="">
  <form id="designinput" name="designinput" autocomplete="off">
  ...
  <td id="equitd"><input type="number" step="1" v-model="n1" class="form-control" autofocus></td>
  ...
  <tr>
  <td id="equitd1"><input type="number" step="1" v-model="d1" class="form-control"></td>
  <td id="equitd2"><input type="number" step="1" v-model="d2" class="form-control"></td>
  </tr>
  </table>
  </div>
  ...
  <table>
  <tr>
  <td><input type="text" v-model="n3" class="form-control" readonly></td>
  <td v-if="equalshow === true" rowspan="2" class="pl-2 pr-2">=</td>
  <td rowspan="2" v-if="x3if === true"><input type="text" v-model="x3" class="form-control" readonly></td>
  <td v-if="n4if === true"><input type="text" v-model="n4" class="form-control" readonly></td>
  </tr>
  <tr>
...
<div class="">
  <h2>Fractions addition steps:</h2>
<p>Select one from simple fraction or mixed fraction</p>
<p>Enter the fraction that you want to add. Once added, click on the calculate button.</p>
</div>
  </div>
</div></div>
...
export default {
  head() {
    // sets document title
    return {
      title: 'Adding fractions calculator - Add mixed fractions',
  // optional; sets final title as "Index Page - My Website", useful for multiple level meta
  // meta tags
  meta: [
      { hid: 'description', name: 'description', content: 'Fractions calculator for addition.' }
  ],
    link: [
    {rel: 'canonical', href: 'https://www.aaryatables.com/math/adding-fractions-calculator'}
    ]
    }
    },
  data(){
...
var n2 = this.n2;
var d2 = this.d2;
var op = this.selected;
if( n1=='' || d1=='' || n2=='' || d2=='' )
{
  alert("Please enter fractions");
  return;
}
if( w1=='-' ) { s1=-1; w1='0'; }
if( w2=='-' ) { s2=-1; w2='0'; }
...
var d4 = d1*d2/this.gcd(d1,d2);
out += '\n= ';
if( op==0 || op==1 ) {
  out += (n1*d4/d1)+'/'+d4+ops+(n2*d4/d2)+'/'+d4;
  if( op==0 ) {
    a = (n1*d4/d1+n2*d4/d2);
    b = d4;
  }
...
var out3 = this.roundresult(this.n3/this.d3);
if( out1!='' ) out1+=' = ';
if( out2!='' ) out2+=' = ';
this.out3 = out3;
  },
...
.adslot_1 { display:none; }
 ...
 h3 { font-size:1.4rem; }
 h4 { font-size:1.2rem; }
 #designinput { padding:20px; max-width:500px; background:#a7c9b8; }
 #designinput td input[type=number] { width:80px; text-align: center; margin:4px auto; }
...
@media all and (max-width: 800px) {
    ...
    #designinput { padding:10px; }
    #designinput td input[type=number] { width:60px; }
    #designinput td input[type=text] { width:60px; }
 }
</style>
